...
// Rate limiting estricto para endpoints de autenticacion (10 req / 15 min; sin limite en test)
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: process.env.NODE_ENV === 'test' ? 10_000 : 10,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: true, message: 'Demasiados intentos de autenticacion, intentalo mas tarde' },
});
 ...
 *   post:
 *     tags: [Auth]
 *     summary: Renovar access token con refresh token (rotation)
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *     responses:
 *       200:
 *         description: Nuevos accessToken y refreshToken
 *         content:
 *           application/json:
 *             schema:
 *               type: object
